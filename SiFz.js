//Subscribe Mau Gaming bantu 100 sub
//Goo100
//Lib
...
//━━━━━━━━━━━━━━━[ CONNECTION 1 ]━━━━━━━━━━━━━━━━━//
		...
		faketeks = '©SIFz メ Bo†z ×͜×' //Ganti Aja Nama Luh
		const isUrl = (url) => {
...
simple.updatePresence(from, Presence.recording)
   ...
   sendButImage(from, pll, `SiFz メ Bo†z ×͜× © 2021`, cewe, [                      
          {
...
//>>>>>>>>>>>>>>>>>>>>[ COMMAND ]<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<//
            ...
            listmenu = [`shop`,`sewabot`,`groupmenu`,`stickermenu`,`downloadmenu`,`islammenu`,`animemenu`,`informationmenu`,`randomtextmenu`,`searchmenu`,`primbonmenu`,`stalkmenu`,`randomimagemenu`,`ownermenu`,`asupanmenu`,`gabut`,`game`,`meme`,`murid`,`djmenu`,`makermenu`,`walisongo`,]
               ...
               const yy = {title: 'SiFz メ Bo†z ×͜×',
                    rows: [
                       {
                        title: `${listmenuu[startnum++]}`,
                        description: `SUBS Mau Gaming`,
                        rowId: `${prefix}${x}`
                      }
                    ]
                   }
...
[`sholawat1`,`sholawat2`,`sholawat3`,`sholawat4`,`sholawat5`,`sholawat6`,`sholawat7`,`sholawat8`,`sholawat9`,] 
               ...
               startnum = 0
               for (let x of listmenu) {
               const yy = {title: 'SiFz メ Bo†z ×͜× ' + nombor++,
                    rows: [
                       {
                        title: `${listmenuu[startnum++]}`,
                        description: ``,
...
𝐃𝐨𝐧'𝐭 𝐒𝐩𝐚𝐦`
oy =`
*_Jasa Sewa Bot By : ©SiFz メ Bo†z ×͜×
*>>>>>>>> *LIST HARGA* >>>>>>>>*
...
case 'original':
simple.updatePresence(from, Presence.recording)
img =  fs.readFileSync('./poto/SiFz.jpg')
teks = `Hai ${pushname} 👋${tampilUcapan}
I'M SiFz メ Bo†z ×͜×`
trans = `BOT INFO
🗿  _*Nama Bot : ${botname}*_
...
costum(buffer, sticker, Verived, `SiFz メ Bo†z ×͜×`)
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
buffer = fs.readFileSync(ran)
costum(buffer, image, Verived, `©_SiFz メ Bo†z ×͜×_ *V : 2.889H*`)
fs.unlinkSync(ran)
})
break
case 'tomp3':
...
fs.unlinkSync(anum)
buffer = fs.readFileSync(ran)
simple.sendMessage(from, buffer, video, { quoted: ftrol, caption: 'Done... SiFz メ Bo†z ×͜×' })
fs.unlinkSync(ran)
})
break
...
simple.sendMessage(`6283856223089@s.whatsapp.net`, options, text, { quoted: ftrol })
...
{ buttonId: `${prefix}qris`, buttonText: { displayText: 'Qris' }, type: 1 }
]
...
sendButton(from, "GOPAY : bit.ly/Gopayscan", faketeks, but, mek)
simple.updatePresence(from, Presence.recording)
img =  fs.readFileSync('./poto/Gopay.jpg')
break
case 'dana':
simple.updatePresence(from, Presence.recording)
but = [
...
sendButton(from, "DANA : 083856223089", faketeks, but, mek)
break
case 'qris':
simple.updatePresence(from, Presence.recording)
but = [
...
sendButton(from, "Qris : bit.ly/QrisAll", faketeks, but, mek)
simple.updatePresence(from, Presence.recording)
img =  fs.readFileSync('./poto/image/Qris.jpg')
break
...
simple.sendMessage(from, buffer, image, {quoted: ftrol, caption: 'SiFz メ Bo†z ×͜×'})
break
case 'hijaber':
...
reply(mess.wait)
buffer = await getBuffer(ini.result.url)
simple.sendMessage(from, buffer, video, {quoted: ftrol, caption: 'SiFz メ Bo†z ×͜×'})
break
case 'ukhti':
...
but = [
        ...
        sendButton(from, teks, '©Created : SiFz メ Bo†z ×͜×', but, mek)
break
 ...
 if (args.length == 0) return reply(`Contoh: ${prefix + command} awok awok`)
                    ...
                    getBuffer(`https://api.lolhuman.xyz/api/textprome2/${command}?apikey=Z-BOT8&text1=${txt1}&text2=${txt2}`).then((gambar) => {
                        simple.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `©SiFz メ Bo†z ×͜× _IMAGE_`, quoted : ftrol})
                    })
                    break
           ...
           case 'cup':
            ...
            simple.updatePresence(from, Presence.recording)
                    ...
                    getBuffer(`https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=${humankey}&text=${ini_txt}`).then((gambar) => {
                        simple.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `©SiFz メ Bo†z ×͜×`, quoted : ftrol})
                    })
                    break
                case 'arcade8bit':
                case 'battlefield4':
...
Sendbutdocument(from, tescok, "© SiFz メ Bo†z ×͜×*", fs.readFileSync('./poto/menu.pdf'), {mimetype:Mimetype.pdf, thumbnail:fs.readFileSync('./poto/image/thumbnaildokumen.jpg'), filename:`SiFz.pdf`, pageCount: 9999999 },[{ buttonId: `${prefix}menu`, buttonText: { displayText: 'OWNER🌺' }, type: 1 }], {quoted:fvid, contextInfo: { mentionedJid: [dtod,otod,stod], forwardingScore: 508, isForwarded: true, externalAdReply:{title:`${tampilUcapan} - ${pushname}\nDownload Script`,mediaType:"2",thumbnail:ofrply,mediaUrl:`https://youtu.be/x-O0WHkv3uc`}}})
break
